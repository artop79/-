...
const MinimalButton = styled(Button)(({ theme }) => ({
  ...
  '&.MuiButton-outlined': {
    borderColor: alpha('#3182fa', 0.5),
    color: '#3182fa',
    '&:hover': {
      borderColor: '#3182fa',
      backgroundColor: alpha('#3182fa', 0.1),
    },
  },
}));
...
const ResumeAnalyzer = ({ minimal, maxWidth = 'md' }) => {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '100%', height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#1a1c27' }}>
      {/* Шапка с логотипом */}
      <Box sx={{ 
        display: 'flex', 
        justifyContent: 'space-between', 
        alignItems: 'center', 
        py: 1, // Уменьшено с p: 2 на py: 1 (меньше вертикальные отступы)
        px: 2,
      ...
      <Box sx={{ flex: 1, p: 3, overflow: 'auto' }}>
        ...
        {/* Здесь уже не нужна кнопка возврата, т.к. она в шапке */}
        ...
        {activeStep === 3 && results && (
          <ResumeAnalysisResults 
            results={results} 
            onReset={handleReset}
            onBack={() => setActiveStep(2)} // Добавляем обработчик для кнопки "Назад"
